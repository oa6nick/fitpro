...
## E2E-проверка интерфейса

```bash
npm run dev                       # терминал 1: сервер + клиент
npm run seed                      # один раз: демо-данные (ЛОКАЛЬНО, деструктивен)
npm run e2e --workspace client    # терминал 2: 11 браузерных проверок
```

Ловит то, чего не видят typecheck и vitest: схлопнувшиеся поля, тап-таргеты
меньше пальца, липкие панели под таб-баром, горизонтальный вылет на 390px,
удаление без подтверждения. Гоняется на `node --test` + `playwright`
(`@playwright/test` не нужен). `BASE_URL` по умолчанию `http://localhost:5173`.

⚠️ Логин ограничен 20 попытками на 15 минут (`server/src/app.ts`) — тесты
логинятся один раз на роль и переиспользуют сессию. В деплой-хук e2e не
включены: на VPS нет браузера.
