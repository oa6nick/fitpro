...
> Код приложения ещё НЕ скаффолжен — это greenfield-заготовка (git + доки + модель данных).
> Структура выше — план; сверяй с реальностью по мере появления кода.

## Команды (планируемые — появятся со скаффолдом)
...
npm run seed             # демо-данные (1 тренер + 4–5 клиентов)
```
...
Симлинк `knowledge-vault/` → `~/Desktop/WORKSPACE/vaults/PROJECT_VAULTS/fitpro`.
