...
/**
 * Горизонтальная phone-галерея + lightbox.
 * Клик по телефону открывает крупно (без лишней кнопки «Открыть»).
 */
    ...
    <>
          ...
          {screens.map((s, i) => (
            <figure key={s.src} className="w-[200px] shrink-0 snap-center sm:w-[220px]">
              <button
                type="button"
                onClick={() => show(i)}
                className="group relative mx-auto block w-full cursor-zoom-in overflow-hidden rounded-[1.85rem] border border-border bg-card p-1.5 text-left shadow-panel ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-panel focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring dark:ring-white/5"
                aria-label={`${s.title}: увеличить`}
              >
                ...
                <span className="pointer-events-none absolute bottom-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-foreground/75 text-background opacity-0 shadow-md transition-opacity group-hover:opacity-100">
              ...
              <figcaption className="mt-3 text-center">
                ...
                <p className={cn("text-sm font-semibold tracking-tight", s.role && "mt-1.5")}>
                  {s.title}
                </p>
      ...
      {open && current && (
        ...
        >
          ...
          >
            <div className="relative w-full max-w-[min(100%,380px)] overflow-hidden rounded-[2rem] border border-white/15 bg-[#111] p-2 shadow-2xl">
